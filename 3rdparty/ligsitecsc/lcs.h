...
public:
	...
	void runLigsite(const std::string& f) {
	  ...
	  for( it = pocket_nr.begin(); it != pocket_nr.end(); it++ )
	    {
	      ...
	      if(index_j > numberofpockets) break;
		  buf.append(QString("ATOM      %1  Fe  PKT Z %2      %3  %4  %5\n").arg(index_j).arg(it->n).arg((it->position).x).arg((it->position).y).arg((it->position).z));

	//	{
	//	  fprintf(pocketfile1, "ATOM  %5d  Fse  PKT Z%4d    %8.3f%8.3f%8.3f\n",  index_j, it->n, (it->position).x, (it->position).y, (it->position).z);
	//	}
